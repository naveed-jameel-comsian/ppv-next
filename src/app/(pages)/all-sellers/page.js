...
const AllSelleres = () => {
    ...
    const handleFilter = (status) => {
        setFilter(status)
        Api.post("/admin/sellers", { status: parseInt(status) })
            .then((res) => {
    ...
    const handleViewPayments = (id) => {
        setModal("payment")
        Api.post("/admin/seller/bills", { user_id : parseInt(id) })
            .then((res) => {
    ...
    return (
        <>
            <AdminHeader addAdmin={false} data={{ breadcrumb }} isFilter="seller" filter={filter} setFilter={handleFilter} />
            <main className="main">
                                    ...
                                    sellers.map((seller, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <span
                                                    key={index}
                                                    onClick={() => handleViewPayments(seller?.id)}
                                                    style={{
                                                        background: "#28a745",
                                                        color: "#fff",
                                                        padding: "5px 10px",
...
const Row = ({ payment, index }) => {
    ...
    const handleUpdatePayment = (status) => {
        ...
        Api.post("/admin/bill/pay", payload).then((res) => {
            toast.success("Update successfully!")
            setStatus(status)
        }).catch((err) => {
            toast.error("Error try again!")
